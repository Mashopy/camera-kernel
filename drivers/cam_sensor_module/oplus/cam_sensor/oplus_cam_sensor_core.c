                        ...
                        uint32_t addr, uint32_t data, unsigned short mdelay)
{
	...
	for( i = 0; i < retry; i++)
	{
		rc = camera_io_dev_write(cci_master_info, &i2c_write, false);
		if (rc < 0) {
                                    ...
                                    void *arg)
{
	...
	case CAM_OEM_IO_CMD: {
		...
		case CAM_OEM_CMD_WRITE_DEV: {
			...
			rc = cam_cci_i2c_write_table(&oem_io_master_info, &write_setting, false);
...
void oplus_cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (slave_info->sensor_id == 0x0766 ||
		...
		if (sensor_version == 0x03) {
			...
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &qsc_tool_write, false);
	...
	if (rc == 0) {
		...
		if (s_ctrl->sensor_initsetting_state == CAM_SENSOR_SETTING_WRITE_INVALID) {
			...
			if (s_ctrl->sensordata->slave_info.sensor_id == 0x689) {
				...
				rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_init_setting, false);
				if(rc < 0) {
